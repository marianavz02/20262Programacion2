...
Debe aparecer únicamente en 1 lugar (o los mínimos indispensables), concretamente en el punto de configuración inicial del sistema (Main) para inyectar la dependencia donde se necesita, de esta manera se evita terminar llenando todo el código de llamadas directas.
...
* **Principio SOLID:** Se cumple el **Principio Abierto/Cerrado (OCP)**, ya que pudimos extender la clase `Compra` agregando un nuevo campo sin necesidad de modificar ni romper el código que ya teníamos escrito. modificar ninguna llamada existente (0 llamadas). Se cumple el **Principio Abierto/Cerrado (OCP)** de SOLID, permitiendo extender el comportamiento mediante el Builder sin alterar el código que ya funcionaba.